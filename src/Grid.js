...
const getBiggestActiveBreakpoint = (breakpoints) => {
  ...
  for (const [breakpoint, active] of Object.entries(breakpoints)) {
    if (active) biggestBreakpoint = breakpoint;
    else break;
  }
...
const buildRowClasses = () => {
  const classes = {};
  classes[RowClass] = buildClass(RowClass, "xs", {
    display: "flex",
    flexDirection: "row",
    flexWrap: "wrap"
  });
...
const buildContainersClasses = (breakpoints, containerMaxWidths) => {
  ...
  classes[FluidClass] = buildClass(FluidClass, "xs", {
    width: "100%",
    ...commonStyles
  });
...
const updateGrid = () => {
  ...
  const breakpointsHaveChanged =
    JSON.stringify(activeBreakpoints) !== JSON.stringify(matchedBreakpoints);
...
const getActiveContainerStyles = (containerClass) => {
  ...
  if (
    properties.breakpoints[containerBreakpoint] >
    properties.breakpoints[biggestActiveBreakpoint]
  )
    return [classes[FluidClass]];
...
const getActiveColumnStyles = (columnClasses) => {
  ...
  const activeColumnStyles = columnClasses
    .filter((responsiveClass) => activeStyles[responsiveClass] !== undefined)
    .map((responsiveClass) => activeStyles[responsiveClass]);

  return activeColumnStyles.length > 0
    ? activeColumnStyles
    : [classes[NoActiveColumnClass]];
};
...
const build = (config = {}) => {
  ...
  const rowClasses = buildRowClasses();
  const columnsClasses = buildColumnsClasses(
    properties.breakpoints,
    properties.columns,
    properties.gutterWidth
  );
  const containersClasses = buildContainersClasses(
    properties.breakpoints,
    properties.containerMaxWidths
  );
  ...
  classes = StyleSheet.create(
    Object.keys(classesDefinition).reduce(
      (ac, current) => ({ ...ac, [current]: classesDefinition[current].style }),
      {}
    )
  );
...
const getActiveStyles = (classes) => {
  const validClasses = classes.filter(
    (_class) => classesDefinition[_class] !== undefined
  );

  const containerClass = validClasses.find((_class) => _class.includes("container"));
  if (containerClass !== undefined) return getActiveContainerStyles(containerClass);

  const rowClass = validClasses.find((_class) => _class.includes("row"));
  if (rowClass !== undefined) return getActiveRowStyles();

  const columnClasses = validClasses.filter((_class) => _class.includes("col"));
  if (columnClasses.length > 0) return getActiveColumnStyles(columnClasses);
